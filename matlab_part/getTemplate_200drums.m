...
savepath = ['./templates/template_200DRUMS_', num2str(windowSize), '_', num2str(hopSize), '.mat'];
...
%get data dir
path200drums = '/Volumes/CW_MBP15/Datasets/200drums/';
allData = recursiveFileList(path200drums, 'wav');
allPath = sort200drums(allData);


tic;
for j = 1:3  
path_inst = allPath{j};
trackNum = length(path_inst);
subW = []; 
concateSegments = [];
    for i = 1:trackNum
